...
- The CD workflow on Github Actions, is not fully tested since require a VM for deployment. However, the underlying scripts for `frontend` and `backend` are tested locally. `worker` scripts is implemented, but untested.
